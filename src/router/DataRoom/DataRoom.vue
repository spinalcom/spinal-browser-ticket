...
export default {
  ...
  methods: {
    ...
    async displayOtherTabs () {
      ...
      this.floorEndpoints = [];
      this.roomEndpoints = [];
      if (this.currentNodeInfo.selectedNode.info.type.get() == "geographicFloor"){
        this.floorEndpoints= await this.getNodeEndpointsInfo(this.currentNodeInfo.selectedNode.info.id.get(),"hasEndPoint")
        const rooms = await SpinalGraphService.getChildren(this.currentNodeInfo.selectedNode.info.id.get(),"hasGeographicRoom");
        //console.log(rooms)
