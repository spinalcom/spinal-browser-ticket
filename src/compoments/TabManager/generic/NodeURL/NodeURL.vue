      ...
      >
        <el-table-column label="URL">
          <div slot-scope="scope">
            {{ scope.row.element.name.get() }}
          </div>
        </el-table-column>
        <el-table-column label="Actions" width="150">
          <div class="actions-documentation" slot-scope="scope">
            <el-tooltip :content="$t('spinal-twin.explore-URL')">
              <el-button
                @click.native="exploreURL(scope.row.element.URL.get())"
                icon="el-icon-search"
                circle
                size="small"
              ></el-button>
            </el-tooltip>
            <el-tooltip :content="$t('spinal-twin.open-URL-in-new-tab')">
              <el-button
                @click.native="openURLInNewTab(scope.row.element.URL.get())"
